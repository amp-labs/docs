...
To create a new record, make a request to the [Write endpoint](/api-reference/write/create-update-upsert-or-delete-records) with `type` being `create`. For example:
...
If you are updating a large number of records, you can make bulk writes by calling the [Write endpoint](/api-reference/write/create-update-upsert-or-delete-records) with `mode` being `bulk` . Because bulk writes are asynchronous, once your request has been successfully received, you'll get a response that contains an Operation ID. You can then use the [GetOperation](/api-reference/operation/get-an-operation) endpoint to poll on the status of this Operation, or view its status on the Ampersand Console.
...
Make a request to the [GenerateUploadURL](/api-reference/upload-url-1/generate-a-signed-url-to-upload-write-data-to) endpoint:
...
The response from the [GenerateUploadURL](/api-reference/upload-url-1/generate-a-signed-url-to-upload-write-data-to) endpoint from step 1 should include a `reference` field which is a URL that starts with `gs://`. Put this URL in the `recordsURL` field of a bulk write request.
...
If you want to get the status of this Operation, you can either look for it on the Ampersand Console, or you can make a request to the [GetOperation](/api-reference/operation/get-an-operation) endpoint. For example:
...
If the Operation has failed, you can get more details about the failure by retrieving the logs related to this Operation using the [ListOperationLogs](/api-reference/operation/list-logs-for-an-operation) endpoint. For example:
