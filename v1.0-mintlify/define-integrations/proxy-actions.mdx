...
All proxy actions expose an API with the base of `https://proxy.withampersand.com`. When making a passthrough call, simply replace the base URL with the Ampersand proxy URL. For example, instead of making a request to `https://subdomain.my.salesforce.com/services/data/v56.0/sobjects/Account`, making the same request to `https://proxy.withampersand.com/services/data/v56.0/sobjects/Account`. To see the API base URLs for a provider you are interested in, find its corresponding file in Ampersand's open-source [connectors library](https://github.com/amp-labs/connectors/blob/main/providers), and look for the `BaseURL` field.
...
* `x-amp-installation-id`: the installation ID, you'll get this from the [CreateInstallation](/reference/installation/create-a-new-installation) or [ImportInstallation](/reference/installation/import-an-existing-installation) API endpoints, or the InstallIntegration React component.
* or both of the following:  
   * `x-amp-integration-name`: the integration name that you wrote in the `amp.yaml` file  
   * `x-amp-group-ref`: the ID for the user group that you used in the [OAuthConnect](/reference/oauth/get-url-for-oauth-flow) or [ImportInstallation](/reference/installation/import-an-existing-installation) API endpoints; or the InstallIntegration React component.
