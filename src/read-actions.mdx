...
To monitor the progress of your backfill, use the [backfill progress API](/reference/installation/get-backfill-progress-for-an-installation-object).

The API returns:
- **recordsProcessed** — How many records have been synced so far; updates as the backfill runs
- **recordsEstimatedTotal** — The estimated total at the start of the backfill (Salesforce and HubSpot only)
- **operationId** — The backfill operation ID for correlation
- **createTime** / **updateTime** — When the backfill started and when progress was last updated
