...
Every webhook message that Ampersand sends to a webhook destination includes a cryptographic signature. This signature lets you confirm that the payload is both authentic and unchanged when it reaches your system.

The signature is sent in the `svix-signature` header. Each destination has its own unique secret, called the "webhook signing secret", which is used to generate the signature.
