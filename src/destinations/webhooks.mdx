...
Every webhook message from Ampersand that is sent to a webhook destination includes a cryptographic signature that allows you to verify the authenticity and integrity of the payload when it is delivered to your destination. This signature is included in the `svix-signature` header. Each destination has a unique secret used to sign the webhooks for that destination and is referred to as the "webhook signing secret".

To verify the signature, you will need to know this secret which is available in the Ampersand dashboard. Once you have the secret, you can use any one of the [Svix libraries](https://docs.svix.com/receiving/verifying-payloads/how) to verify the signature for a webhook message.
...
3. Click on the destination you want to view the secret for
4. Click on **"Click here to view the secret"**
