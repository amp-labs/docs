...
- **subscribeEventType**: this is the normalized event type string, which can be: `create`, `update`, or `delete`
  ...
  "result": [
    {
      ...
      "providerEventType": "contact.PropertyChange",
      "raw": { ... },
      "rawEvent": { ... }
    }
  ]
...
By default, Ampersand will send you at most 1000 webhook messages per second across all your destinations. If you wish the change this limit, please contact `support@withampersand.com`. Alternatively, you can set the delivery mode to [on request](/define-integrations/read-actions#on-request) to only get webhook messages when you request for them via an API call. 
