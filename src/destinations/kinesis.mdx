...
When new data is read from a SaaS instance via a [Read Action](/read-actions), Ampersand can send the payload to your AWS Kinesis stream.

## Prerequisites

Before setting up a Kinesis destination, ensure you have:
...
| **Partition key template** | string | No | [JMESPath](https://jmespath.org) template for partition key (default: `metadata.eventId`) |
| **Endpoint URL** | string | No | Custom Kinesis endpoint (for testing or LocalStack) |
...
Kinesis uses partition keys to distribute messages across shards. You can customize the partition key using a JMESPath template.

**Default partition key template:** `metadata.eventId`

This uses the event ID as the partition key, ensuring all records from the same event are sent to the same shard.

### Custom partition key examples

Since all message data is wrapped in a `data` object, you need to prefix your JMESPath expressions with `data.`:
...
**Use operation ID for unique partitioning:**
```
data.operationId
```

<Info>
JMESPath is a query language for JSON. See [jmespath.org](https://jmespath.org) for the full specification.
</Info>
...
1. Verify stream name and region are correct
2. Test AWS credentials:
   ...
   aws kinesis describe-stream \
     ...
     --region your-region
   ```
3. Check IAM permissions include `kinesis:PutRecord` and `kinesis:PutRecords`

## Limitations

- **Message size**: Individual records cannot exceed 1 MB (AWS Kinesis limit)
