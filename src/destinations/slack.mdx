...
<Warning>
Slack accepts roughly **16 KB** per message. Larger payloads are rejected by Slack, so be careful using Slack destinations for data delivery — a single record or batch that exceeds this limit will not be delivered. For anything beyond small payloads, use a [webhook](/destinations/webhooks), [Kinesis](/destinations/kinesis), or [S3](/destinations/s3) destination instead.
</Warning>
