...
In asynchronous mode, the API validates the request & immediately returns an operation ID. The API then processes the write in the background. If the write fails, Ampersand will automatically keep retrying the operation for up to 1 hour. This lets you avoid having to implement your own retry logic. If needed, the retry policy can be configured to retry a write for up to 48 hours.
...
To check the status of an async write operation, use the [Get Operation endpoint](/reference/operation/get-an-operation) with the operation ID after the write request is made, ideally after the deadline has passed.
