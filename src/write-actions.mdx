...
When writing to providers that have nested data structures, you can use [JSONPath bracket notation](/object-and-field-mapping#nested-field-mapping) in your record keys:
...
Ampersand will automatically transform these into the nested structure the provider API expects.

If the write action uses `inheritMapping: true` and the read action has nested field mappings, the same mappings will be applied in reverse when writing. See [Writing nested data](/object-and-field-mapping#writing-nested-data) for details.
