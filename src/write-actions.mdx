...
Ampersand can automatically groups multiple single-record write requests together and executes them as a batch operation. This is useful when you want to reduce the amount of API quota that you are consuming in your customer's SaaS instance.
...
Each batch uses a the max delay deadline set by the first record in that batch. This ensures records don't wait indefinitely.
