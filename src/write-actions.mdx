...
### Passing additional headers to providers when creating or updating records
...
Batch writes support both [synchronous and asynchronous modes](#write-modes). Note that batch writes follow an **all-or-nothing** approach: if any record in the batch fails to write, the entire batch operation fails and no records are written.
...
The auto-batch policy automatically groups multiple single-record write requests together and executes them as a batch operation. This is useful when you're sending many individual write requests and want to optimize them into batches for better performance and efficiency.

<Info>Auto-batch policy is currently only available for asynchronous mode and single-record writes (not batch writes).</Info>

When you enable auto-batch policy, Ampersand will:
...
2. **Execute when ready**: The batch executes when either:
   - **100 records** have been accumulated (immediate execution), or
   - **Maximum delay time** expires (executes even if fewer than 100 records)

Each batch uses a per-batch deadline based on when the first record in that batch arrived. This ensures records don't wait indefinitely.

#### Configuration

The auto-batch policy has two configuration options:

- **`maxDelayMinutes`** (required): The maximum time to wait before executing the batch, even if it hasn't reached 100 records. Minimum: 1 minute, Maximum: 1440 minutes (24 hours).
- **`retryDeadlineHours`** (optional): The maximum time to keep retrying the batch write operation if it fails. If not specified, defaults to 1 hour. Maximum allowed is 48 hours.

#### Example: Using auto-batch policy

To enable auto-batch policy, include `autoBatchPolicy` in your write request:
...
In this example:
- Records will be batched together for up to **5 minutes**
- If 100 records accumulate before 5 minutes, the batch executes immediately
- If fewer than 100 records accumulate, the batch executes after 5 minutes
- If the batch write fails, Ampersand will retry for up to **2 hours**

#### Monitoring auto-batch operations
...
- `success`: The batch write completed successfully (may include partial success with some records failing)
- `failure`: The batch write failed

## Write modes

Write actions support two modes: `synchronous` and `asynchronous`. The mode determines how the API handles your write request and when you receive a response. These modes apply to both single record writes and batch writes.
...
For single-record writes, you can also use the [auto-batch policy](#auto-batch-policy) to automatically group multiple requests together and execute them as a batch.
