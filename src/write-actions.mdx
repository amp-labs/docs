...
<Info>
Deletion must be enabled for the object in the installation config. See [Enabling deletion for an object](#enabling-deletion-for-an-object) to enable it when creating or updating an installation.

This is currently supported for HubSpot and Salesforce.
</Info>

To delete a record, call the same [Write endpoint](/reference/write/write-records) with `type` set to `delete`. Provide the record `id` in the `record` object; no other fields are required. Delete operations are **synchronous only** and support **single-record** deletes.
...
A successful delete returns a response with `result.success` and the `recordId` that was deleted.
...
By default, delete operations are not allowed for an object. To allow your app to delete records via the Write API, you must enable deletion for that object in the installation config. Set `deletionSettings.enabled` to `true` for the object when creating or updating an installation.

Disable deletion by setting `deletionSettings.enabled` to `false` or omitting `deletionSettings` (deletion is off by default).

You can use any of these APIs; expand the option you need:
