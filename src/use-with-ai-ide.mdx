...
To run the MCP server locally, run
```
npx @amp-labs/mcp-docs-server@latest
```

This starts the server at http://localhost:3001. If the port is already taken, it will pick another port.

Next, add the following to your MCP configuration file. Its location depends on the client you are using. See [Client-specific configuration](#client-specific-configuration).
