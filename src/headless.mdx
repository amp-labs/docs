...
<Note>
For more advanced config management: including getting/setting read/write objects in the config and managing a local draft copy, use the [`useLocalConfig()`](#local-config-management) hook.
</Note>
...
const {
  ...
  getReadObject: (objectName: string) => {
    ...
    isAlwaysEnabled: () => boolean;  // Object is marked `enabled: always` in amp.yaml (v2.18.0+)
  },
...
<Note>
Objects marked [`enabled: always`](/manifest-reference) in your `amp.yaml` are read for every installation, so they can't be turned off. Starting in `v2.18.0`, `setDisableRead` does nothing for them (and logs a warning), and `removeObject` removes only their write config. Use `useManifest().getReadObject(objectName).isAlwaysEnabled()` to find these objects, for example to show them as selected and locked in your UI.
</Note>
...
- [useConfig](#local-config-management): manage installation configuration state
