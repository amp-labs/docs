...
#### Setting write feature in isolation

You can configure write settings for individual fields using these methods:
...
#### Setting write features together

The `WriteObjectHandlers` returned by `writeObject` also supports setting all field settings at once:

```typescript
// extended shape of WriteObjectHandlers (returned by `writeObject` function above).
{
  setFieldSettings: (params: { 
    fieldName: string; 
    settings: {
      writeOnCreate?: 'always' | 'never';
      writeOnUpdate?: 'always' | 'never';
      default?: {
        stringValue?: string;
        integerValue?: number;
  ...
  }) => void;  // Configure field-level write settings
  getFieldSettings: (fieldName: string) => FieldSettings | undefined;  // Get field settings
...
You can configure when a field should be written to during create and update operations. This helps prevent accidentally overwriting fields that customers have manually modified in their SaaS product.
...
#### Setting default values (alternative)

You can set default values for fields that will be applied whenever that value is missing from the write request. We support string, boolean and number values.
...
Here's an example that combines multiple write features:
...
The [headless demo write app](https://github.com/amp-labs/demo-headless-write) uses a Salesforce integration, and extends the basic demo app with:
