...
// Return values of `useLocalConfig` hook.
{
  ...
  ensureObject: (objectName: string) => void;  // Initialize read object with manifest defaults (v2.12.0+)
...
The `ensureObject` and `removeObject` functions provide a simple way to add or remove objects from the draft config. Supported in `@amp-labs/react` `v2.12.0+`.

`ensureObject` initializes a read object in the draft config with default values from the manifest (e.g., required fields are automatically selected). It is idempotent, so it is safe to call multiple times for the same object.
...
function ObjectManager() {
  ...
  // Ensure the "contact" object is initialized with manifest defaults.
  // Required fields will be pre-selected automatically.
  config.ensureObject("contact");

  // Ensure another object
  config.ensureObject("account");
