...
<Note>
As of `v2.12.1`, `useCreateInstallation` automatically populates `config.provider` from the integration object if it is not explicitly set in the config. This means you no longer need to manually set the provider in most cases.
</Note>
...
The `getReadObjects` function returned by `useManifest` returns all read objects defined in the manifest. This is useful when you need to iterate over all objects rather than fetching them individually by name. Supported in `@amp-labs/react` `v2.12.1+`.
