...
The `AmpersandProvider` supports both API key and JWT authentication through the `getToken` option. Only one authentication method can be used at a time.
...
The `getToken` function should return a promise that resolves to a valid JWT token. 

<Note>
For more details on JWT authentication details, setup, and implementation, see [JWT Authentication](/jwt-auth).
</Note>
