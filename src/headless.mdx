...
<Note>
As of `v2.12.0`, `useCreateInstallation` automatically populates `config.provider` from the integration object if it is not explicitly set in the config. This means you no longer need to manually set the provider in most cases.
</Note>
...
const {
  getReadObjects: () => HydratedIntegrationObject[],  // Get all read objects from manifest (v2.12.0+)
...
The `getReadObjects` function returned by `useManifest` returns all read objects defined in the manifest. This is useful when you need to iterate over all objects rather than fetching them individually by name. Supported in `@amp-labs/react` `v2.12.0+`.

```tsx
const { getReadObjects } = useManifest();

// Get all read objects from the manifest
const allReadObjects = getReadObjects(); // HydratedIntegrationObject[]

// Iterate over all read objects to build your UI
...
// Return values of `useLocalConfig` hook.
{
  ...
  removeObject: (objectName: string) => void;  // Remove object from all actions (v2.12.0+)
...
`setEnableRead` initializes a read object in the draft config and sets it as enabled. It is idempotent, so it is safe to call multiple times for the same object. Note that setting a selected field (`setSelectedField`) or field mapping (`setFieldMapping`) will also automatically enable the read object in the config, so `setEnableRead` is primarily useful when you want to enable a read object without setting any fields, or to re-enable an object that was previously disabled.

`setDisableRead` sets the read object as disabled (pausing reads without removing it from the config).

`removeObject` removes an object from all actions (both read and write) in the draft config.

The naming mirrors the write pattern: `config.readObject("contacts").setEnableRead()` is consistent with `config.writeObject("contacts").setEnableWrite()`.
...
function ObjectManager() {
  ...
  // Enable reading for the "contact" object (initializes with defaults)
  config.readObject("contact").setEnableRead();

  // Enable another object
  config.readObject("account").setEnableRead();

  // Disable reading for an object (sets disabled flag, keeps config)
  config.readObject("account").setDisableRead();

  // Remove an object from all actions (read and write)
