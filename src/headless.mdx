...
The Headless UI Library provides a series of react hooks that manage connections, installations, 
and other configuration data with queries and mutation hooks. The hooks may be used together 
or independently from the [prebuilt UI components](/embeddable-ui-components).
...
The `InstallationProvider` needs to wrap any code that interacts with a particular installation. This means that if your UI needs to handle multiple installations (e.g. one for Asana and one for Zendesk), then you need 2 instances of `InstallationProvider`, one that wraps the code for Asana set up, and one that wraps the code for Zendesk set up.
...
For example, the `useCreateInstallation` hook is used to creates a new Installation, it returns the following:
...
> For now, you can only access your manifest's Read Actions. Subscribe and Write Actions are coming soon. Please note that you still create an Installation with Subscribe and Write Actions by constructing the Config yourself and using [`useCreateInstallation`](headless#creating%2C-updating%2C-and-deleting-installations).
...
The `getCustomerFieldsForObject` function returned by `useManifest` allows you to retrieve the standard and custom fields that exists in your customer's SaaS instance for a particular object.

```tsx
  const { getCustomerFieldsForObject } = useManifest();

  // Get all the fields that exists on the customer's Account object,
...
[The fields mapping page of the demo app](https://github.com/amp-labs/headless-basic-app/blob/main/src/components/FieldMappingTable/FieldMappingTable.tsx) provides a full example for using `useManifest` to build the UI for customers to configure the installation.

## Config management

Managing the [Config](/terminology#config) that keeps track of each customer's preference for how the integration behaves can be complex with deep nested objects and need to manage this state locally.

The `useConfig` hook is provided to simplify local state management of the config object by providing flexible utilities to manipulate the config through a set of setters and getters. It maintains a draft state that you can modify before committing changes to the installation.
...
This is a basic example that hard-codes a Config and does not allow users to modify it.
...
This is an example for how to use helper functions to more easily construct a write config, so you do not have to create the full config object manually. See [advanced write use cases](/write-actions#advanced-use-cases) for an explanation of these features.
...
[View the source code on GitHub](https://github.com/amp-labs/headless-basic-app)

![Dynamic Fields Table Example](/images/headless/headless-basic-app-table.png)

The [headless demo app](https://github.com/amp-labs/headless-basic-app) uses a Salesforce integration, and includes:
...
- [useConnections](#connection-management): manage when to show [ConnectProvider](/embeddable-ui-components#connect-provider)
- [useManifest](#usemanifest): manage objects and metadata
- [useConfig](#config-state-management): manage installation configuration state'
...
If you do not want your users to be able to modify or configure the installation, you can build a pre-defined configuration flow using the code snippet below. This is helpful if you do not want your user to be able to modify which objects and fields your integration reads and writes, and you do need them to do any field mappings.
