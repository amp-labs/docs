...
Based on [new security restrictions](https://help.salesforce.com/s/articleView?id=005132365&type=1) on *uninstalled connected apps* starting September 2025, integrations fall into the following categories:
- **Installed App**
...
- To verify which profiles have access to an object's fields:
  ...
  4. Check field-level access for the relevant profiles. [Salesforce reference docs are here.](https://help.salesforce.com/s/articleView?id=platform.checking_field_accessibility_for_a_particular_field.htm&type=5)
![Fields Access](/images/customer-guides/salesforce-5.png)

**Permissions for Ampersand's actions**:
- Users performing `Subscribe`, `Read` actions must have the following **object permissions** set in their profile:
  * View All Records
  * View All Fields
- Users performing `Write` action need these permissions:
  * Modify All Records
...
**Permissions for Ampersand's actions**:
- Users performing `Subscribe` actions must have the following **user permissions** set in their profile:
