...
3. If the integration uses a Connected App (instead of an External Client App that you install on your Salesforce org), then you need to enable one of the following permissions.
  * `Approve Uninstalled Connected Apps`   
  * `Use Any API Client`

Note you will typically only see one of these two permissions show up as an option. This is based on whether you have [API Access Control](https://help.salesforce.com/s/articleView?id=xcloud.security_api_access_control_about.htm&type=5) enabled. You only need to enable one of these permissions - whichever one that you see in the UI.
