...
<Tabs>
  ...
  ![Install for All Users](/images/customer-guides/salesforce-eca/install-for-all-users.png)

  You will see a progress screen while the installation completes.

  ![Installing in progress](/images/customer-guides/salesforce-eca/installing-in-progress.png)

  Once complete, you should see a confirmation screen:

  ![Installation Complete](/images/customer-guides/salesforce-eca/installation-complete.png)
  ...
  ![Installed Packages](/images/customer-guides/salesforce-eca/installed-packages.png)
  ...
  > **Note:** Salesforce [no longer supports creation of new Connected Apps](https://help.salesforce.com/s/articleView?id=005228017&language=en_US&type=1). If the integration you are using still relies on a Connected App, the steps below apply. For new integrations, ask the developer to migrate to an External Client App.
...
1. `API Enabled`
2. [Subscribe Action Permissions](#subscribe-action-permissions) if the integration includes real-time Subscribe Actions.

<Tabs>
  <Tab title="External Client App">

  If the integration uses an External Client App, no additional system permissions are needed beyond `API Enabled` (and Subscribe Action permissions if applicable). The package installation handles app authorization automatically.

  </Tab>
  <Tab title="Connected App (Legacy)">

  If the integration uses a Connected App (instead of an External Client App that you install on your Salesforce org), then you need to enable one of the following permissions:
  * `Use Any API Client` if you see it. (You will only see this option show up if your organization has enabled [API Access Control](https://help.salesforce.com/s/articleView?id=xcloud.security_api_access_control_about.htm&type=5)).
