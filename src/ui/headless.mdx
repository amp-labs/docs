...
or independently from the @amp-labs/react [embeddable UI components](/embeddable-ui-components).
...
The headless react lib is currently in the same package as our [embeddable UI components](/embeddable-ui-components).
...
### Ampersand Provider
The Ampersand Provider is needed at the project level and is also needed for all hooks in headless lib. See [Ampersand Provider](/embeddable-ui-components#ampersand-provider) for more details.
...
function App() {
  return (
    ...
    <AmpersandProvider options={{ apiKey: apiKey, projectId: projectId }}>
      ...
      >
        {/* Your Custom Installation Component */}
        <MyComponent />
      </InstallationProvider>
    </AmpersandProvider>
  );
...
- `connection`: The current [Connection](/terminology#connection) object, or null if not connected
...
## useManifest
The `useManifest` hook provides access to the integration's [manifest](/terminology#revision) data, which is defined in your `amp.yaml` file. This hook allows you to:

- Access read objects and their required/optional fields as defined in your manifest
- Retrieve field metadata from the connected provider (e.g. Salesforce, Hubspot)
- Build dynamic field mapping interfaces based on available fields

The manifest data comes from two sources:

1. Your `amp.yaml` file which defines the [integration's structure](/quickstart#define-the-integrations)
2. The connected provider's metadata about available fields and objects

This hook is essential for building dynamic configuration interfaces that adapt to the available fields in your customer's SaaS instance.

Retrieves and manages manifest data from `amp.yaml` and the customer's SaaS instance:
- access to [readObject](/read-actions), required fields, optional fields
...
```tsx
  ...
  // Selected object (pre-defined selected object)
...
[View the field mapping example](https://github.com/amp-labs/headless-basic-app/blob/main/src/components/FieldMappingTable/FieldMappingTable.tsx)


## Config State Management 
Managing the configuration state that needs to be passed into the [Installation](/terminology#installation) can be complex with deep nested objects and need to manage this state locally. 
...
### Basic App Example
[View the demo app on GitHub](https://github.com/amp-labs/headless-basic-app)
...
- connects with Salesforce through [ConnectProvider](/embeddable-ui-components#connect-provider)
...
![Salesforce Field Mappings Example](/images/headless/headless-basic-app-select.png)

### Pre-defined flow (no user input needed)
