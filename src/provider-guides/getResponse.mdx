...
- [Read Actions](/read-actions), including full historic backfill and incremental read. Incremental read is supported for `contacts` using provider-side filtering; all other objects use connector-side filtering based on their `createdOn` timestamp.
...
- **Write support**: `contacts` and `campaigns` support create and update. `tags` support create and delete only — the update operation is deprecated by GetResponse.
- **contacts**: Must be associated with an existing campaign on creation (a `campaignId` is required).
- **Incremental read**: `contacts` uses provider-side filtering via `query[createdOn][from/to]`. All other readable objects use connector-side filtering based on the `createdOn` field in each record.
- **Timestamp format**: GetResponse uses `+0000` (no colon in the UTC offset) rather than the standard `Z` suffix. The connector handles this automatically.
- Single-record operations only (no bulk write).
