...
To define an integration for Outplay, create a manifest file that looks like this:

```YAML
# amp.yaml
specVersion: 1.0.0
integrations:
  - name: Outplay-integration
    displayName: My Outplay Integration
    provider: outplay
    proxy:
      enabled: true
```
...
- Start making [Proxy Calls](/proxy-actions), and Ampersand will automatically attach the correct header required by Basic Auth. Please note that this connector's base URL is `https://{{.workspace}}-api.outplayhq.com`.
