...
- **Read-only objects**: `contacts`, `segments`, `singlesends`, `field_definitions`, `verified_senders`, `senders`, `bounces`, `blocks`, `spam_reports`, `unsubscribes`, `invalid_emails`, `categories`, and `subusers`
- **Incremental read**: Not supported.
