...
If you are looking to build an interactive, customer-facing Salesforce integration, use the standard [Salesforce connector](/provider-guides/salesforce) instead.
...
This connector uses OAuth 2.0 JWT Bearer authentication with an RSA key pair. There is no interactive login — your customer's Salesforce administrator will pre-authorize a designated integration user on a Connected App, and Ampersand will sign JWT assertions on their behalf to obtain access tokens.
...
Your customer's Salesforce administrator creates an **External Client App** (or configures an existing **Connected App**) with JWT Bearer flow enabled, uploads an X.509 certificate, and assigns a permission set that pre-authorizes the integration user. The full steps are documented in the [customer guide](/customer-guides/salesforce-jwt).

<Note>Since Spring '26, Salesforce no longer allows creating new **Connected Apps** — the customer guide covers both the new **External Client App** flow and the legacy Connected App flow for customers who already have one. See [Salesforce's announcement](https://help.salesforce.com/s/articleView?id=005228017&language=en_US&type=1) for details.</Note>
...
- Create a manifest file like the [Salesforce example](https://github.com/amp-labs/samples/blob/main/salesforce/amp.yaml), using `salesforceJWT` as the provider.
