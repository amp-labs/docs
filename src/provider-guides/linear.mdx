...
5. Enter the previously obtained Client ID in the **Client ID** field and the Client Secret in the **Client Secret** field. For Scopes you can start with `write` (which grants the app write access to all objects) or use more targeted scopes `issues:create` or `comments:create` (check [here](https://linear.app/developers/oauth-2-0-authentication#redirect-user-access-requests-to-linear) for more details)
