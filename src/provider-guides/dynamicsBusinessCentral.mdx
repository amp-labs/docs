...
<Warning>Dynamics requires certain scopes to have the resource prefixed to the scope, which in this case is the consumer's instance URL. Ampersand will prefix the instance URL for you so you can only provide the scope name (i.e. `.default`).</Warning>
<Warning>In order to receive a refresh token and be able to access your customers' Microsoft Dynamics Business Central APIs without reauthenticating, you must include the `offline_access` permission when registering the app with Ampersand</Warning>
