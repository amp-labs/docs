...
The Copper connector supports reading to the following objects:
...
#### Custom Fields
In addition to standard fields, the connector fully supports any **custom fields** you have created through the Copper API.
Custom fields are treated the same as built-in fields: you can query or write to them alongside standard fields.
All custom fields are exposed with the prefix `custom_field_`.
