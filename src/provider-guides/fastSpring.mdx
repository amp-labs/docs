...
- **Read behavior**: [Read Actions](/read-actions) list supported object collections (accounts, orders, products, subscriptions, and event lists). Responses may be full records or ID-only lists; the connector normalizes rows into a consistent shape. Incremental read is supported for `events-processed` and `events-unprocessed` where the API allows date filters on read parameters.
- **Write support**: `accounts` and `products` support **create** and **update**. `orders` and `subscriptions` support **update** only. Single-record operations only (no bulk write).
- **Commands**: FastSpring also exposes **command**-style operations (for example subscription cancel or product deactivation) that are not the same as creating or updating a record in a collection. They are not documented here as Write or Delete actions. See the [FastSpring API reference](https://developer.fastspring.com/reference/getting-started-with-your-api) for command endpoints, or use [Proxy Actions](/proxy-actions) to call them directly.
