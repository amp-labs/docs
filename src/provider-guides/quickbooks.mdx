...
- [Proxy Actions](/proxy-actions), using the base URL `https://quickbooks.api.intuit.com` for production apps, or `https://sandbox-quickbooks.api.intuit.com` for development apps (see Sandbox below).

## Sandbox

For development and testing, QuickBooks provides a sandbox environment. To use Proxy Actions against the sandbox, specify `quickbooksSandbox` as the provider (instead of `quickbooks`) in your integration. Read Actions and Write Actions are coming soon.

```yaml
specVersion: 1.0.0
integrations:
  - name: quickbooksSandboxProxy
