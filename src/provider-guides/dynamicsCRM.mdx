...
<Warning>You must include the `offline_access` permission when registering the app with Ampersand. Without this permission, Ampersand will not receive a refresh token and your customers will need to reauthenticate each time their access token expires.</Warning>
