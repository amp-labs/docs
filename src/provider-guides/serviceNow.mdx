...
- [Read Actions](/read-actions), including full historic backfill. Please note that incremental read is supported for `lead`, `contact`, `consumer`,`case`,`change` and `account`  only. For all other objects, a full read of the Apollo instance will be done for each scheduled read. 
