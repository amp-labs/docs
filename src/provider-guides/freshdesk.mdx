...
- [Read Actions](/read-actions), including full historic backfill. Please note that incremental read is supported only for `companies`, `tickets` and `contacts` currently. For all other objects, a full read of the Aha! instance will be done per scheduled read.
