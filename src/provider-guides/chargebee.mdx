...
- [Read Actions](/read-actions), including full historic backfill. Please note that incremental read is supported only for `coupons`, `credit_notes`, `customers`, `hosted_pages`, `invoices`, `item_prices`, `items`, `orders`, `payment_sources`, `quotes`, `subscriptions`, `transactions`, `usages` and `virtual_bank_accounts`. For all other objects, a full read of the Chargebee instance will be done per scheduled read.
