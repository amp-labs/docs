...
- [Read Actions](/define-integrations/read-actions), including full historic backfill. Please note that incremental
read is not supported, a full read of the Google instance will be done for each scheduled read.
- [Write Actions](/define-integrations/write-actions).
- [Proxy Actions](/define-integrations/proxy-actions), using the base URL `https://www.googleapis.com`.


### Supported Objects

The Google connector supports reading from and writing to the following objects:
...
* [settings](https://developers.google.com/calendar/api/v3/reference/settings#resource-representations)
