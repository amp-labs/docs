...
Additionally, the Google **Mail (Gmail)** module supports:
- [Read Actions](/read-actions), including historical backfill. Please note that incremental read and pagination
**are only supported for the Mail "Drafts", "Messages", and "Threads" objects**; otherwise, a full read of the Google instance will be performed for each scheduled run.
...
### Supported Mail objects

The Google connector supports reading from the following **Mail (Gmail)** module objects:
