...
_[1] `drafts` supports nested fields, e.g. `"$['message']['payload']['body']"`._

_[2] `messages` supports nested fields, e.g. `"$['payload']['body']"`._
...
Grant Ampersand's Gmail Subscribe service account two resource-scoped roles on your topic:
...
Both bindings are scoped to this single topic, and grant no access elsewhere in your project.
