...
- [Read Actions](/define-integrations/read-actions), including full historic backfill. Please note that incremental read is supported only for `conversations`, `contacts`, `activity_logs` and `tickets` currently. For all other objects, a full read of the Intercom instance will be done per scheduled read.
