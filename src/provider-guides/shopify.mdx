...
- **For installs across many independent Shopify organizations (Ampersand consumers)**: use **Public distribution** and publish an App Store listing.
...
## Share your Shopify app with Ampersand consumers (third-party stores)

To use your integration with third-party Shopify merchants (stores you don’t control), those merchants must be able to install and authorize your Shopify app. This installation step is what grants OAuth access for that merchant’s store.

To support installs across many independent Shopify organizations (like Ampersand consumers), your app must use **Public distribution**.

If you don’t want your app to be searchable in the Shopify App Store, set your app listing to **limited visibility**. Merchants can still install it from the listing URL, but it won’t appear in search results or category pages.

### Builder (Account A): make the app installable (public, limited visibility)
...
4. Tell them about your business add add a credit card to pay $19 one-time app store registration fee.
...
7. After Shopify approves your app, share the **App Store listing URL** with your Ampersand consumers so they can install it.
   ![Copy App Store listing URL](/images/provider-guides/shopify/shopify-app-approved.png)

### Ampersand consumer (Account B): install and authorize

1. Open the App Store listing URL shared by the builder.
2. Install the app on your Shopify store and approve the requested access scopes.
3. Complete the OAuth flow in the builder’s app (via the embedded [Install Integration](/embeddable-ui-components#install-integration) component).
