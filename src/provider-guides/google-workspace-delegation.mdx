...
<Note>Unlike OAuth-based Google connections, this connector does not use a [Provider App](/define-integrations#create-a-provider-app). The service account JSON key contains all the credentials needed, so Client ID and Client Secret are not required.</Note>

### Creating a connection per user

Each connection on Ampersand represents one user's access. Because domain-wide delegation can impersonate any user in the domain, you create one connection per Workspace user you want to sync — each connection shares the same service account key but carries a different user email. Builders typically create these programmatically via the [Generate Connection API](/api/connection/generate-a-new-connection) in bulk.
...
- Collect your customer's service account key, then create one connection per Workspace user (service account key + user email).
