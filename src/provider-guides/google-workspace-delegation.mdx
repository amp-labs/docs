...
Each connection on Ampersand represents one user's access. Because domain-wide delegation can impersonate any user in the domain, you create one connection per Workspace user you want to sync — each connection shares the same service account key but carries a different user email. Builders typically create these programmatically via the [Generate Connection API](/reference/connection/generate-a-new-connection) in bulk.

Here's an example API request body for creating the connection for a user with email `user@example.com`:
