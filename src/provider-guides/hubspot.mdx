...
#### Event Occurrences
The connector reads HubSpot event occurrences (the [Events API](https://developers.hubspot.com/docs/api-reference/latest/events/retrieve-events/get-events)).
Connector exposes occurrences as separate *custom objects* whose names are constructed by combining a fixed *prefix* with the *event type* name.
