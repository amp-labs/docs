...
| `enabled` | String | No | If set to `always`, Ampersand reads this object for every installation, even if the customer never selects it. It's added to each installation's config when the installation is created, and to existing installations when a deploy adds the object or newly marks it `always`. Customers can't disable it: a config that sets `disabled: true` on it is rejected. The object must have at least one `requiredFields` entry, and required fields can't be mappings the customer fills in (a `mapToName` without a `fieldName`). The prebuilt UI components show the object as selected and don't let customers deselect it. |
