...
<Note>
If applicable, some payloads will contain `config` field nested in `data` field which contains the complete installation configuration object including read, write, subscribe, and proxy settings. `installation.updated` includes an additional `lastConfig` field with the previous configuration.
When the config object is too large, we will generate signed url for the config content downloadable in JSON file.
