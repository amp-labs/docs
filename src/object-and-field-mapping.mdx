...
Providers may return data with nested structures. Ampersand supports mapping from and to nested fields using **JSONPath bracket notation**, allowing you to extract values from nested objects or write values into nested structures.

Nested field paths use the format `$['parentField']['childField']['subChildField$With.Special"Characters"']`. Bracket notation ensures that field names containing dots, spaces, or other special characters are correctly parsed.

Value mappings also support bracket notation.

### Reading nested data

When a provider returns nested data, you can use field mappings to extract nested values into mapped fields that may be flat or nested themselves.
...
**With this field mapping configuration in an installation config:**
```JSON
{
  "selectedFieldMappings": {
    "contactEmail": "$['userInfo']['email']",
    "contactName": "$['userInfo']['name']",
    "$['myAppInfo']['currentStatus']": "status"
  }
}
```
...
The same mappings will be converted to their original structure when writing to a provider.
