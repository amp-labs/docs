...
The `getToken` function should return a promise that resolves to a valid JWT token by making a call to your backend. For detailed information on how to generate JWT tokens, see [JWT Authentication](/jwt-auth).
...
It returns an object with fields isLoaded, indicating whether the API call has resolved, and isIntegrationInstalled, indicating whether the integration has already been installed by somebody in the group.
