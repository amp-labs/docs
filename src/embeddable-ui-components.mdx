...
Please note that each [group](/terminology#group) is able to install the integration once, so if someone else in the same `groupRef` has already installed the integration, then the user will not be able to install the same integration again.
