...
description: "Embed Ampersand's React components with AmpersandProvider to let customers install integrations, connect providers, and manage installations in your app."
---
...
<Steps>
  <Step title="Your end user connects their SaaS instance.">
    ![ConnectProvider initial screen](/images/connect-provider-v2.13.7.png)
  </Step>
  <Step title="They configure the integration's objects, fields, and mappings.">
    ![InstallIntegration configuration screen](/images/install-integration-v2.13.7.png)
  </Step>
</Steps>
...
![](/images/install-wizard-demo.gif)

<Warning>The wizard mode is currently in beta preview. The `variant="wizard"` prop is hidden and does not appear in the public TypeScript declarations as of `@amp-labs/react` v2.13.1. See the supported features table below for details on what is currently available.</Warning>
...
<InstallIntegration
  ...
  groupRef={teamId}
  groupName={teamName}
  // @ts-expect-error -- needed since hidden as an internal prop
  variant="wizard"
/>
```
...
| [Field mapping `default`](/object-and-field-mapping#field-mapping-1) | No |
...
<table>
<thead>
<tr><th width="50%">Connect</th><th width="50%">Success</th></tr>
</thead>
<tbody>
<tr>
<td style={{verticalAlign: 'top'}}>

![ConnectProvider initial state](/images/connect-provider-v2.13.7.png)

</td>
<td style={{verticalAlign: 'top'}}>

![ConnectProvider connected state](/images/connect-provider-success-v2.13.7.png)
...
### Why did installing or updating fail with a field error?

When a customer creates or updates an installation, Ampersand runs a [sample read](/read-actions#install-time-validation) against the provider before saving. If a selected field isn't visible to the connected user, or a field name doesn't exist on the object, the install is rejected and the component shows the provider's error message. Your customer can fix it in the flow — for example, by granting field access — instead of hitting a broken sync later.

This runs automatically on `@amp-labs/react` **2.13.5 and later**. Older versions show only a generic error, so upgrade for the detailed messages. See [Install-time validation](/read-actions#install-time-validation) for the full behavior.
