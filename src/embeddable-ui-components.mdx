...
When you are ready to ship your integrations to product, we highly recommend that you move to JWT authentication, as it is more secure than using API keys in the frontend. JWT tokens are time-bound and also ensure that your users only has access to their own integration data.
...
The `getToken` function should return a promise that resolves to a valid JWT token. Your backend should generate this token using your Ampersand API key and include any necessary user context. For detailed information on how to implement this in your backend, see [JWT Authentication](/jwt-auth).
