...
<Warning>The wizard mode is currently in beta preview. The `variant="wizard"` prop is hidden and does not appear in the public TypeScript declarations as of `@amp-labs/react` v2.13.1. Please see [supported features](#supported-features) for details on what is currently available.</Warning>
...
| Object mappings | Yes |
| Value mappings | No |
| Field default value mappings | No |
| Dynamic field mappings (`fieldMapping` prop) | No |
| Schedules | No |
...
The user picks which objects to sync from the provider. A "Write Enabled" toggle appears when the object is selected and is supported via the deployed `amp.yaml`.
...
For each selected object, the user walks through sub-pages. Users can click back to a previously configured object.
