...
<Note>
  The Ampersand MCP server is in beta. It may change in non backwards-compatible ways. (Although we are very serious about semantic versioning.)
...
Here is how you can connect to the Ampersand hosted MCP server from your AI agent or from another MCP client (such as Cursor or OpenAPI Responses API). Please see [client-specific configuration](/use-with-ai-ide#client-specific-configuration) for how to configure the most popular MCP clients.
