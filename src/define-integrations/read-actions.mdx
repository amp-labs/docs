...
Ampersand will do reads at the schedule you specify in `amp.yaml`. If there are any new or updated records since our last read, Ampersand will send you webhook messages.

Please refer to our [OpenAPI spec](https://github.com/amp-labs/openapi/blob/main/webhook/webhook.yaml) for the schema of the webhook message. For the sample integration above, here's what a sample webhook message might look like for the Contact object:
