...
2) **URL**: If a single record is over ~300KB, it usually becomes too big to be sent over a webhook, and Ampersand is unable to split it any further. In this case, the result won't be included inline. Instead, the webhook message will contain a secure download URL in `resultInfo.downloadUrl`.
