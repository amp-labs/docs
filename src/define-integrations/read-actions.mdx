...
Ampersand will read data on the schedule you specify in `amp.yaml`. If there are any new or updated records since our last read, Ampersand will send you webhook messages.

### Chunking and Result Types
#### Chunking
If the data Ampersand has read is too large for a single webhook, it is automatically split into multiple webhook messages. You do not need to add any special logic to handle this.
...
2) **URL**: If a single row of data is over ~300KB, it usually becomes too big to be sent over a webhook, and Ampersand is unable to split it any further. In this case, the result won't be included inline. Instead, the webhook message will contain a secure download URL in `resultInfo.downloadUrl`.
...
Here's some pseudo-code to illustrate how you can get the result from a webhook:
