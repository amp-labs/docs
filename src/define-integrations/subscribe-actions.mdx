...
<Note>
  ...
  - They are currently only supported for Salesforce and HubSpot. And, requires a Salesforce admin to install the integration.
...
## HubSpot subscribe action private preview

This section describes how you can use subscribe actions to receive near-instant webhooks as events occur for HubSpot during the private preview mode.

To use Subscribe Actions for HubSpot in the private preview, follow these steps:

- Configure your connected app in HubSpot.
- Update the amp.yaml configuration file.
- Redeploy your integration.
- Monitor and verify webhook data.

### Updating your HubSpot connected app
...
6.  Enter the Target URL:
  ...
  ```
  - Replace *PROJECT_ID* with your Ampersand project ID.
  - Replace *INTEGRATION_ID* with your integration's ID.
...
### Modifying your amp.yaml file

During the private preview, you must define a Read Action for any event that requires a Subscribe Action and remove the schedule from the Read Action to limit it to historic backfill only.

Here's how you must modify your amp.yaml file to define the subscribe action:
...
### Re-deploy integration

After modifying your amp.yaml file, redeploy the integration using the following command:

```bash
amp deploy <my-folder> --project <my-project>
```

To see the webhook messages about events happening in your HubSpot, go to [HubSpot webhook message](/destinations/webhooks#hubspot-webhook-message).
