...
- **updateEvent**: triggers when any existing record is modified. You can list specific fields to watch in `requiredWatchFields`. The `watchFieldsAuto: all` setting ensures that you subscribe to all field changes. **Note** Only one of `requiredWatchFields` or `watchFieldsAuto` should be provided.
