...
If you are also using Read Actions and have defined [field mappings](/define-integrations/read-actions#field-mappings). Your Write Actions can use the same mappings if you add `inheritMapping` to your manifest file. This means when you call our Write API, we will ensure that we are writing back to the appropriate field that the customer has mapped. 
