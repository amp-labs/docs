...
To see the API base URLs for a provider you are interested in, find its corresponding file in Ampersand's open-source [connectors library](https://github.com/amp-labs/connectors/blob/main/providers), and look for the `BaseURL` field.
...
* `x-amp-installation-id`: the installation ID, you'll get this from the [CreateInstallation](/reference/installation/create-a-new-installation) or [ImportInstallation](/reference/installation/import-an-existing-installation) API endpoints, or the [InstallIntegration](/embeddable-ui-components#install-integration) component's success callback functions.
