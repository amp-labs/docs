...
- The schedule frequency for Ampersand to check for schema changes. The minimum frequency is once per hour, and the default is once every 24 hours.
...
For each event type, set `enabled: always` to receive notifications for that type of change.
...
Ampersand provides observability around this feature by creating an operation in your project for each schema change detection attempt made.

To view these, in your Ampersand dashboard, navigate to the project, integration and installation where you'd like to inspect a schema change operation and look for a 'Schema Watch' operation.

<Frame caption="Schema Change Operation">![Schema Change Operation](/images/schema-watch-op.png)</Frame>