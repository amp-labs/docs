      ...
      objects:
        - objectName: account
          ...
          updateEvent:
            enabled: always
            requiredWatchFields:
              - notes # this is mapped field , defined in read action, that will be selected by user
              - phone
              - accountName # this is mapped from read action
...
For an example of a webhook message delivered according to the above `amp.yaml`, please see [Subscribe action webhooks](/destinations/webhooks#subscribe-action-webhooks) section.
